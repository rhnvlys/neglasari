@php
    $navs = [
        [
            'route' => Auth::user()->hasRole('Kepala Desa') ? 'kades.dashboard' : 'admin.dashboard',
            'label' => 'Dashboard',
            ...
            'roles' => ['Super Admin', 'Admin Desa', 'Kepala Desa']
        ],
        [
        ...
        [
            'route' => Auth::user()->hasRole('Kepala Desa') ? 'kades.attendances.index' : 'admin.attendances.index',
            'label' => 'Log Absensi',
            'icon' => '<svg class="w-6 h-6 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>',
            'roles' => ['Super Admin', 'Admin Desa', 'Kepala Desa']
        ],
        [
            'route' => Auth::user()->hasRole('Kepala Desa') ? 'kades.leave-requests.index' : 'admin.leave-requests.index',
...
@foreach($navs as $nav)
    @if(Auth::user()->hasAnyRole($nav['roles']))
        @php
            // Memeriksa kesamaan nama route atau prefix-nya
            $isActive = false;
            try {
