...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="bg-white rounded-2xl shadow-lg p-6 border border-neglasari-border">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-neglasari-border">
            <h2 class="text-xl font-bold text-neglasari-dark">Edit Jadwal Kerja: {{ $schedule->name }}</h2>
        ...
        <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $schedule->name) }}" required class="w-full px-4 py-2 border border-neglasari-border rounded-xl focus:ring-neglasari-accent focus:border-neglasari-accent">
            ...
            <div class="p-4 bg-neglasari-bg rounded-xl border border-neglasari-border">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="time" name="check_in_start" id="check_in_start" value="{{ old('check_in_start', \Carbon\Carbon::parse($schedule->check_in_start)->format('H:i')) }}" required class="w-full px-3 py-2 border border-neglasari-border rounded-xl">
                    </div>
                    <div>
                        ...
                        <input type="time" name="check_in_time" id="check_in_time" value="{{ old('check_in_time', \Carbon\Carbon::parse($schedule->check_in_time)->format('H:i')) }}" required class="w-full px-3 py-2 border border-neglasari-border rounded-xl font-bold text-green-700">
                    </div>
                    <div>
                        <label for="check_in_end" class="block text-xs font-semibold text-neglasari-text mb-1">Batas Akhir Masuk</label>
                        <input type="time" name="check_in_end" id="check_in_end" value="{{ old('check_in_end', \Carbon\Carbon::parse($schedule->check_in_end)->format('H:i')) }}" required class="w-full px-3 py-2 border border-neglasari-border rounded-xl">
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 bg-neglasari-bg rounded-xl border border-neglasari-border">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="time" name="check_out_start" id="check_out_start" value="{{ old('check_out_start', \Carbon\Carbon::parse($schedule->check_out_start)->format('H:i')) }}" required class="w-full px-3 py-2 border border-neglasari-border rounded-xl">
                    </div>
                    <div>
                        ...
                        <input type="time" name="check_out_time" id="check_out_time" value="{{ old('check_out_time', \Carbon\Carbon::parse($schedule->check_out_time)->format('H:i')) }}" required class="w-full px-3 py-2 border border-neglasari-border rounded-xl font-bold text-blue-700">
                    </div>
                    <div>
                        <label for="check_out_end" class="block text-xs font-semibold text-neglasari-text mb-1">Batas Akhir Pulang</label>
                        <input type="time" name="check_out_end" id="check_out_end" value="{{ old('check_out_end', \Carbon\Carbon::parse($schedule->check_out_end)->format('H:i')) }}" required class="w-full px-3 py-2 border border-neglasari-border rounded-xl">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" name="late_tolerance_minutes" id="late_tolerance_minutes" value="{{ old('late_tolerance_minutes', $schedule->late_tolerance_minutes) }}" min="0" required class="w-full px-4 py-2 border border-neglasari-border rounded-xl focus:ring-neglasari-accent focus:border-neglasari-accent">
                @error('late_tolerance_minutes') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex flex-wrap items-center gap-6 pt-2">
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_workday" value="1" {{ old('is_workday', $schedule->is_workday) ? 'checked' : '' }} class="sr-only peer">
                ...
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_default" value="1" {{ old('is_default', $schedule->is_default) ? 'checked' : '' }} class="sr-only peer">
                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-neglasari-main"></div>
                    <span class="ml-3 text-sm font-semibold text-neglasari-text">Jadwal Default</span>
                </label>

                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $schedule->is_active) ? 'checked' : '' }} class="sr-only peer">
                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-neglasari-main"></div>
