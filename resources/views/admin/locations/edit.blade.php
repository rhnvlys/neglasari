...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="bg-white rounded-2xl shadow-lg p-6 border border-neglasari-border">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-neglasari-border">
            <div>
                <h2 class="text-xl font-bold text-neglasari-dark">Edit Lokasi Kantor: {{ $location->name }}</h2>
                <p class="text-xs text-neglasari-text-secondary mt-1">Sesuaikan titik koordinat dan radius toleransi lokasi kantor</p>
            </div>
        ...
        <form action="{{ route('admin.locations.update', $location) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $location->name) }}" required class="w-full px-4 py-2.5 border border-neglasari-border rounded-xl focus:ring-neglasari-accent focus:border-neglasari-accent shadow-sm">
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="3" required class="w-full px-4 py-2.5 border border-neglasari-border rounded-xl focus:ring-neglasari-accent focus:border-neglasari-accent shadow-sm">{{ old('address', $location->address) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="radius_meters" id="radius_meters" value="{{ old('radius_meters', $location->radius_meters) }}" min="1" required class="w-full px-4 py-2.5 border border-neglasari-border rounded-xl focus:ring-neglasari-accent focus:border-neglasari-accent shadow-sm">
                        ...
                        <div>
                            ...
                            <input type="number" name="maximum_accuracy_meters" id="maximum_accuracy_meters" value="{{ old('maximum_accuracy_meters', $location->maximum_accuracy_meters) }}" min="1" required class="w-full px-4 py-2.5 border border-neglasari-border rounded-xl focus:ring-neglasari-accent focus:border-neglasari-accent shadow-sm">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <label class="block text-sm font-bold text-neglasari-dark">📍 Titik Lokasi di Peta</label>
                    ...
                    <p class="text-[11px] text-gray-500 italic">* Klik atau geser pin pada peta untuk menyesuaikan posisi kantor secara presisi.</p>
                </div>
            </div>
            ...
            <div x-data="{ showManual: false }" class="bg-gray-50 rounded-2xl p-4 border border-neglasari-border">
                ...
                <div x-show="showManual" x-cloak class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude', $location->latitude) }}" class="w-full px-3 py-2 border border-neglasari-border rounded-xl font-mono text-xs focus:ring-neglasari-accent focus:border-neglasari-accent">
                    </div>

                    <div>
                        <label for="longitude" class="block text-xs font-semibold text-neglasari-text mb-1">Longitude (Bujur)</label>
                        <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude', $location->longitude) }}" class="w-full px-3 py-2 border border-neglasari-border rounded-xl font-mono text-xs focus:ring-neglasari-accent focus:border-neglasari-accent">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3 pt-2">
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="requires_photo" value="1" {{ old('requires_photo', $location->requires_photo) ? 'checked' : '' }} class="sr-only peer">
                ...
                <br>
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="allow_outside_radius" value="1" {{ old('allow_outside_radius', $location->allow_outside_radius) ? 'checked' : '' }} class="sr-only peer">
                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-neglasari-main"></div>
                    <span class="ml-3 text-sm font-semibold text-neglasari-text">Izinkan Absen di Luar Radius (Dinas Luar)</span>
                </label>
                <br>
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $location->is_active) ? 'checked' : '' }} class="sr-only peer">
                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-neglasari-main"></div>
            ...
            <div class="pt-4 flex justify-end space-x-3 border-t border-neglasari-border">
                ...
                <button type="submit" class="px-6 py-2.5 bg-neglasari-main text-white font-semibold rounded-xl hover:bg-neglasari-accent transition shadow-md">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
