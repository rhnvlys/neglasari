...
<div class="space-y-6">
    <div class="bg-white rounded-2xl shadow-lg p-6 border border-neglasari-border">
        <h2 class="text-xl font-bold text-neglasari-dark mb-4">Statistik Absensi Hari Ini</h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-neglasari-bg rounded-xl p-4 text-center">
                <p class="text-3xl font-bold text-neglasari-main">{{ $stats['present'] }}</p>
                <p class="text-sm text-neglasari-text-secondary">Hadir</p>
            </div>
            <div class="bg-neglasari-bg rounded-xl p-4 text-center">
            ...
            <div class="bg-neglasari-bg rounded-xl p-4 text-center">
                <p class="text-3xl font-bold text-blue-500">{{ $stats['leave'] }}</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-neglasari-border">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.leave-requests.index') }}" class="text-xs font-semibold text-neglasari-main hover:underline">Lihat Semua</a>
            </div>
            <div class="space-y-4">
                @forelse($pendingLeaves as $leave)
                    <div class="flex items-center justify-between p-3 bg-neglasari-bg rounded-xl">
                        ...
                        <a href="{{ route('admin.leave-requests.show', $leave) }}" class="px-3 py-1 bg-white text-neglasari-main hover:bg-gray-100 text-xs font-bold rounded-lg border border-gray-200">Proses</a>
                    </div>
                @empty
